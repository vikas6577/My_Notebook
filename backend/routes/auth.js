...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      //instead of sending user data(id password...) we send authtoken in which are data(user id,jwtsecret(our secret message) is taken and converted to token and send to the user so that in future if someone comes with the same token we can retrieve its userid)
      // res.json({ user });
    } catch (error) {
